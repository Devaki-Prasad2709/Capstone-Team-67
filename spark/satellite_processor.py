"""Topic-specific transformations for satellite-imagery."""
...
SCHEMA = StructType(
    [
        ...
        StructField("original_format", StringType()),
        StructField("stream_format", StringType()),
...
def process(raw: DataFrame) -> DataFrame:
    """Validate satellite metadata and discard the large Base64 payload."""
    return (
        raw.select(
        ...
        .filter(
            F.col("image_id").isNotNull()
            & (F.col("source") == "satellite")
            & F.lower("original_format").isin("tif", "tiff")
            & (F.lower("stream_format") == "jpg")
            & (
